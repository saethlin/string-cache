...
pub(crate) struct Set {
    buckets: Box<[Option<NonNull<Entry>>; NB_BUCKETS]>,
}
...
impl Set {
    pub(crate) fn insert(&mut self, string: Cow<str>, hash: u32) -> NonNull<Entry> {
        ...
        unsafe {
            let mut ptr: Option<NonNull<Entry>> = self.buckets[bucket_index];

            while let Some(entry) = ptr.take() {
                let entry = entry.as_ptr();
                if (*entry).hash == hash && *(*entry).string == *string {
                    if (*entry).ref_count.fetch_add(1, SeqCst) > 0 {
                        return NonNull::new_unchecked(entry as *const Entry as *mut Entry);
                    }
        ...
        let entry = unsafe {NonNull::new_unchecked(Box::into_raw(Box::new(Entry {
    ...
    pub(crate) fn remove(&mut self, ptr: *mut Entry) {
        ...
        let mut current: Option<NonNull<Entry>> = self.buckets[bucket_index];

        while let Some(entry_ptr) = current {
            let entry_ptr: *mut Entry = entry_ptr.as_ptr();
            if entry_ptr == ptr {
                mem::drop(mem::replace(&mut current, unsafe {
                    (*entry_ptr).next_in_bucket.take()
                }));
                break;
            }
            current = unsafe { (*entry_ptr).next_in_bucket };
        }
    }
}
